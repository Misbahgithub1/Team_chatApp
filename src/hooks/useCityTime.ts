...
export function useCityTime(city: string, country?: string) {
  ...
  useEffect(() => {
    ...
    async function fetchTime() {
      try {
        // Step 1: Get city coordinates from OpenStreetMap geocoding API
        const geoRes = await fetch(
          ...
          { 
            ...
            headers: { "User-Agent": "TeamChatApp/1.0" }
          }
        );
        ...
        // Step 2: Fetch only local time from Open-Meteo
        ...
        // Time is in current_weather.time
